...
class Email {
   ...
   * @param Text $title The email title may vary depending the receiver.
   * @param Text $message The email message may vary depending the receiver.
   * @param Url $appointmentLink This link is going to enable the receiver to make changes
   * to the appointment record.
   * @param string $externalLink
   * @param EmailAddress $recipientEmail The recipient email address.
   * @param Text $icsStream Stream contents of the ICS file.
   * @throws Exception
   */
    public function sendAppointmentDetails(
        ...
        $replaceArray = [
            ...
            'NoReply Message' => $this->framework->lang->line('noreply_message')
        ];

        if (boolval($service['vide_active']))
        {
            $replaceArray['External Link'] = $this->framework->lang->line('external_link');
            $ext_appointment_link = (new Url($externalLink))->get();
            $replaceArray['$ext_appointment_link'] = $ext_appointment_link;
            $replaceArray['Document Info'] = $this->framework->lang->line('document_info');
            $html = file_get_contents(__DIR__ . '/../../application/views/emails/appointment_vide_active_details.php');
        }
        else
        {
     ...
     * @param EmailAddress $recipientEmail The email address of the email recipient.
     * @param Text $reason The reason why the appointment is deleted.
     * @throws Exception
     */
     ...
     *
     * @param NonEmptyText $password Contains the new password.
     * @param EmailAddress $recipientEmail The receiver's email address.
    ...
    protected function _createMailer()
    {
        ...
        $mailer->setFrom($this->config['smtp_user']);
