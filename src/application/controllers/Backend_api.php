...
use \EA\Engine\Exceptions\UsernameExistsException;
...
class Backend_api extends CI_Controller {
    ...
    public function ajax_save_appointment()
    {
        try
        {
            ...
            {
                // appschedweb customization
                // drag & drop appointment from backend calendar: customer is undefined
            ...
            if ($this->input->post('appointment_data'))
            {
                ...
                $res_save_vide = $this->_save_vide_appointment($appointment, $customer, $service);
                $vide_url = $res_save_vide['vide_url'];
                $vide_url_operator = $res_save_vide['vide_url_operator'];
            }
            ...
            try
            {
                ...
                if ((bool)$send_customer === TRUE)
                {
                    $email->sendAppointmentDetails($appointment, $provider,
                        ...
                        $customer_message, $customer_link, $vide_url, new Email($customer['email']), new Text($ics_stream));
                }
                ...
                if ($send_provider == TRUE)
                {
                    $email->sendAppointmentDetails($appointment, $provider,
                        ...
                        $provider_message, $provider_link, $vide_url_operator, new Email($provider['email']), new Text($ics_stream));
                }
        ...
        catch (GuzzleException $e)
        {
            ...
            $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode(['exceptions' => [exceptionToJavaScript($e)]]));
        }
    }
    ...
    public function ajax_delete_appointment()
    {
        try
        {
            ...
            if (boolval($service['vide_active']))
                $this->_delete_vide_appointment($this->input->post('appointment_id'));
    ...
    public function ajax_save_service()
    {
        try
        {
            ...
            // appschedweb customization: controllo sul numero di sportelli consentiti (inferiore o uguale a 'company_max_services')
            $is_new = true;
            ...
            $this->load->model('settings_model');
            if ($is_new && count($services) >= $this->settings_model
                    ->get_setting('company_max_services')){
                throw new Exception($this->lang->line('service_max_number_reached'));
            }
    ...
    public function ajax_save_admin()
    {
        ...
        catch (UsernameExistsException $exc)
        {
            $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'exceptions' => [exceptionToJavaScript(new UsernameExistsException($this->lang->line('username_already_exists')))]
                ]));
        }
        catch (Exception $exc)
        {
    ...
    public function ajax_save_provider()
    {
        try
        {
            ...
            {
                // TODO add column working_plan in ea_services con json del working plan
                // ricercare il  working plan del primo (se è presente) service associato al provider
                // e poi $provider['settings']['working_plan'] = con il working plan dello sportello (service)
                $this->load->model('settings_model');
    ...
    public function ajax_save_settings()
    {
        try
        {
            if ($this->input->post('type') == SETTINGS_SYSTEM)
            {
                ...
                $this->_update_vide_layout($settings);
                $this->_update_vide_carousel($settings);
            }
            else
            {
    ...
    /**
     * [AJAX] start a VIDE video call.
     *
     * This method redirect to the VIDE url.
     *
     * Required POST Parameters:
     *
     * - int $_POST['appointment_id'] The appointment id to be video call.
     */
    public function ajax_start_videocall()
    {
        try
        {
            if ($this->privileges[PRIV_APPOINTMENTS]['videocall'] == FALSE)
            {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            // recupero external address dalla tabella di abbinamento tra id easy appointments e id esterni
            $ext_appointments_id = $this->appointments_model->get_ext_id($this->input->post('appointment_id'), 'VIDE');

            log_message('INFO', 'ajax_start_videocall - id_easyappointments: ' . $this->input->post('appointment_id')
                . ', id_ext_appointments: ' . $ext_appointments_id);

            $this->load->library('Vide_sync');
            $this->load->helper('vide');
            $body_json_app = $this->vide_sync->get_appointment($ext_appointments_id);
            $vide_url_operator = videocall_operator_url($body_json_app->virtual_desk, $ext_appointments_id);

            $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode(['vide_url_operator' => $vide_url_operator]));
     ...
     * @throws Exception
     */
    private function _save_vide_appointment($appointment, $customer, $service)
    {
        $this->load->model('appointments_model');
        $this->load->model('customers_model');
        ...
        $operator_first_name = $this->customers_model->get_value('first_name', $appointment['id_users_provider']);
        $operator_last_name = $this->customers_model->get_value('last_name', $appointment['id_users_provider']);

        // per capire se si tratta di una modifica oppure di un inserimento si tenta di recuperare
        // l'appuntamento dalla tabella di abbinamento degli id appuntamenti
        $ext_appointments_id = $this->appointments_model->get_ext_id(
            $appointment['id'],
            $this->config->item('vide_ext_system_name')
        );
        log_message('DEBUG', 'ext_appointments_id:' . $ext_appointments_id);

        if (!boolval($service['vide_active']))
        {
            if ($ext_appointments_id == '')
            {
                log_message('INFO', 'Insert new appointment VIDE NOT active, return');
                return [ 'vide_url' => null, 'vide_url_operator' => null ];
            }
            else
            {
                log_message('INFO', 'Update appointment previous VIDE active');
                $this->vide_sync->delete_appointment($ext_appointments_id);

                // delete appointment dalla tabella di decodifica ea_appointments_id_extrel
                $this->db->where('easy_appointments_id', $appointment['id']);
                if (!$this->db->delete('ea_appointments_id_extrel'))
                {
                    throw new Exception('Could not delete ea_appointments_id_extrel record.');
                }
                return [ 'vide_url' => null, 'vide_url_operator' => null ];
            }
        }
        ...
        if ($ext_appointments_id == '')
        {
            log_message('INFO', 'Insert new appointment');;
            try
            {
                $body_json_vd = $this->vide_sync->virtual_desk();
                log_message('DEBUG', 'virtual_desk_id:' . $body_json_vd->{'virtual_desk_id'});
                log_message('DEBUG', 'conference_id:' . $body_json_vd->{'conference_id'});

                // URL da inserire nella mail spedita all'utente e memorizzo le date inizio e fine video conferenza */
                $vide_url = videocall_user_url($body_json_vd);

                $vide_appointment_id = $this->vide_sync->add_appointment($appointment, $customer,
                    $service, $body_json_vd, $operator_first_name, $operator_last_name);

                $vide_url_operator = videocall_operator_url($body_json_vd, $vide_appointment_id);

                $app_id_ext = [
                    'easy_appointments_id' => $appointment['id'],
                    'ext_appointments_id' => $vide_appointment_id,
                    'ext_system_name' => $this->config->item('vide_ext_system_name')
                ];

                if (!$this->db->insert('ea_appointments_id_extrel', $app_id_ext))
                {
                    throw new Exception('Could not insert ea_appointments_id_extrel record.');
                }
                unset($app_id_ext);
                return [ 'vide_url' => $vide_url, 'vide_url_operator' => $vide_url_operator ];
            }
            catch (GuzzleException $e)
            {
                log_message('error', $e->getMessage());
                log_message('error', $e->getTraceAsString());
                log_message('error', 'Rilevata GuzzleException => rimozione dell\'appuntamento inserito, id: ' . $appointment['id']);
                $this->appointments_model->delete($appointment['id']);
                throw $e;
            }
        }

        log_message('INFO', 'Update appointment, id VIDE: ' . $ext_appointments_id);
        $body_json_app = $this->vide_sync->get_appointment($ext_appointments_id);

        $this->vide_sync->update_appointment($appointment, $customer, $service, $body_json_app,
            $operator_first_name, $operator_last_name);

        return [
            'vide_url' => videocall_user_url($body_json_app->virtual_desk),
            'vide_url_operator' => videocall_operator_url($body_json_app->virtual_desk, $ext_appointments_id)
        ];
    }

    /**
     * In caso di service (sportello) con videoconferenza, si invocano le API
     * per eliminare l'appuntamento dal DB Vide
     * @param $appointment_id
     * @throws Exception
     * @throws GuzzleException
     */
    private function _delete_vide_appointment($appointment_id)
    {
        $this->load->model('appointments_model');
        $this->load->library('Vide_sync');
        ...
        log_message('INFO', 'id_easyappointments: ' . $appointment_id
            . ', id_ext_appointments: ' . $ext_appointments_id);

        $this->vide_sync->delete_appointment($ext_appointments_id);

        // cancello l'appuntamento dalla tabella di decodifica ea_appointments_id_extrel
        $this->db->where('easy_appointments_id', $appointment_id);
        if (!$this->db->delete('ea_appointments_id_extrel')) {
            throw new Exception('Could not delete ea_appointments_id_extrel record.');
        }
    }
     ...
     * @throws GuzzleException
     */
    private function _update_vide_layout($settings)
    {
        $color = null;
        $logo = null;
        foreach ($settings as $setting)
        {
            if ($setting['name'] == 'company_logo')
                $logo = 'data:' . $setting['value'];
            if ($setting['name'] == 'company_color')
                $color = $setting['value'];
        }

        if (isset($color) || isset($logo))
        {
            $this->load->library('Vide_sync');
            $layout = $this->vide_sync->get_layout();

            $color = isset($color) ? $color : (isset($layout->{'color'}) ? $layout->{'color'} : '0072e5');
            $logo = isset($logo) ? $logo : (isset($layout->{'logo'}) ? $layout->{'logo'} : '');

            if (isset($layout) && !empty($layout))
            {
                try {
                    // Attenzione: la API layout restituisce sempre un json non vuoto,
                    // o quello del tenant o quello di default.
                    $this->vide_sync->update_layout($logo, $color);

                } catch (GuzzleException $e) {
                    log_message('error', 'GuzzleException: ' . $e->getCode() . ' - '
                        . $e->getMessage());
                    if ($e->getCode() == 400)
                        // Layout for tenant not found
                        $this->vide_sync->add_layout($logo, $color);
                }
            }
            else
                $this->vide_sync->add_layout($logo, $color);
     ...
     * @throws GuzzleException
     */
    private function _update_vide_carousel($settings)
    {
        foreach ($settings as $setting)
        {
            if ($setting['name'] == 'company_vide_carousel')
                $carousel = json_decode($setting['value'], true);
        }
        if (isset($carousel))
        {
            $this->load->library('Vide_sync');
            try
            {
                $this->vide_sync->get_carousel();
                $this->vide_sync->delete_carousel();
            }
            catch (GuzzleException $e)
            {
                if ($e->getCode() == 404)
                    log_message('info', 'GuzzleException: ' . $e->getMessage());
                else
                    throw $e;
            }

            if (!empty($carousel))
                $this->vide_sync->add_carousel($carousel);
        }
    }
}
