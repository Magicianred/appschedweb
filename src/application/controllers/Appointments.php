...
class Appointments extends CI_Controller {
    ...
    public function cancel($appointment_hash)
    {
        try
        {
            ...
            if (boolval($service['vide_active']))
                $this->_delete_vide_appointment($appointment['id']);
        ...
        {
            $view = [
                'message_title' => $this->lang->line('appointment_not_cancelled_title'),
                'message_text' => $this->lang->line('appointment_not_cancelled'),
                'message_icon' => base_url('assets/img/error.png')
            ];
        }
        else
    ...
    public function ajax_register_appointment()
    {
        ...
        catch (GuzzleException $e)
        {
            ...
            $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'exceptions' => [exceptionToJavaScript($e)]]));
        }
    }

    /**
     * [AJAX] Get public holiday
     *
     * Get an array with the public holiday stored in the ea_settings
     *
    ...
    public function ajax_get_holidays()
    {
        ...
        try {
            ...
            $holiday_array = $this->appointments_model->get_holidays_array();
            ...
            $this->output
                    ->set_content_type('application/json')
                    ->set_output(json_encode(['holiday_array' => $holiday_array]));
        } catch (Exception $exc) {
            $this->output
                    ->set_content_type('application/json')
                    ->set_output(json_encode([
    ...
    private function _save_vide_appointment($appointment, $customer, $service)
    {
        ...
        log_message('DEBUG', 'ext_app:' . $ext_appointments_id);
        ...
        # VIDE active
        if ($ext_appointments_id == '') {
            log_message('INFO', 'Insert nuovo appuntamento');
            try
            {
