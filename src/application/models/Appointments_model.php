...
class Appointments_Model extends CI_Model {
    ...
    public function exists($appointment)
    {
        ...
        {
            throw new Exception(lang('Not_all_req_fields_provided') . ':'
    ...
    protected function _insert($appointment)
    {
        ...
        {
            throw new Exception(lang('Could_not_ins_appointment'));
        }
    ...
    protected function _update($appointment)
    {
        ...
        {
            throw new Exception(lang('Could_not_upd_appointment_rec'));
        }
    }
    ...
    public function find_record_id($appointment)
    {
        ...
        {
            throw new Exception(lang('Could_not_find_app_rec_id'));
        }
    ...
    public function validate($appointment)
    {
        ...
        {
            throw new Exception(lang('app_start_date_invalid'));
        }

        if ( ! validate_mysql_datetime($appointment['end_datetime']))
        {
            throw new Exception(lang('app_end_date_invalid'));
        }
    ...
    public function get_batch($where_clause = '', $aggregates = FALSE)
    {
        if ($where_clause != '')
        {
            $this->db->where(str_replace('"', "'", $where_clause));
        }
    ...
    public function add_unavailable($unavailable)
    {
        ...
        if ($start > $end)
        {
            throw new Exception(lang('start_date_before_end_error'));
        }
	...
	/** S.F. 17/0872020
     * Recupera il corrispondente appointments_id del sistema esterno dalla tabella ea_appointments_id_extrel
     *
     * @param int $appointment_id l'id appuntamento easyappointments.
     *
     * @return varchar Returns l'id appuntamento del sistema esterno.
     *
     * @throws Exception If $appointment_id argumnet is invalid.
     */
    public function get_ext_id($appointment_id,$ext_system_name)
    {
       
		 return $this->db
            ->select('ext_appointments_id')
            ->from('ea_appointments_id_extrel')
            ->where('easy_appointments_id =',  $appointment_id)
            ->where('ext_system_name =', $ext_system_name )
            ->get()
            ->row()
            ->ext_appointments_id;		
      ...
      /**
     * Get holidays array from  from ea_holidays  table.
     *
     * @example $this->Model->ajax_get_holidays();
     *
     *
     * @return array Returns an array build from the rows of the database.
     */
    public function get_holidays_array()
    {
         ...
         return $this->db
            ->select('value')
            ->from('ea_settings')
            ->where('name =', 'company_closing_days')       
            ->get()
            ->row()
            ->value;		
