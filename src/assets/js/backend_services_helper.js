...
(function () {
    ...
    ServicesHelper.prototype.bindEventHandlers = function () {
        ...
        $('#save-service').click(function () {
            const service = {
                ...
                vide_active: $('#service-vide-type').val(),
                additional_info: $('#service-additional-info').val()
            };
            ...
            instance.checkLimitServiceReached()
        });
     ...
     * Si disabilita il bottone di aggiunta service (Aggiungi sportello) se il numero di services
     * (sportelli) è maggiore o uguale al valore di company_max_services.
     * @param services
     */
    ServicesHelper.prototype.checkLimitServiceReached = function (services = GlobalVariables.services.length){
        const disabled = services >= GlobalVariables.max_services;
        $('#add-service').prop('disabled', disabled);
    }
    ...
    ServicesHelper.prototype.delete = function (id) {
        ...
        $.post(postUrl, postData, function (response) {
            ...
            this.checkLimitServiceReached(GlobalVariables.services.length - 1);
            this.resetForm();
    ...
    ServicesHelper.prototype.display = function (service) {
        ...
        $('#service-vide-type').val(service.vide_active);
        $('#service-additional-info').val(service.additional_info);

        const categoryId = (service.id_service_categories !== null) ? service.id_service_categories : 'null';
