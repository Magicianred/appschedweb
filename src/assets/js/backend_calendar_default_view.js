...
(function (exports) {
    ...
    function _bindEventHandlers() {
        ...
        /**
         * Event: Popover Video Button "Click"
         *
         * chiama il metodo per il recupero della URL di connessione alla video chiamata
         */
        $calendarPage.on('click', '.video-popover', function () {
            $(this).parents().eq(2).remove(); // hide the popover
          /*  if (lastFocusedEventData.data.is_unavailable == false) {*/
                var buttons = [
                    {
                        text: 'OK',
                        click: function () {
                            var url = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_start_videocall';
                            var data = {
                                csrfToken: GlobalVariables.csrfToken,
                                appointment_id: lastFocusedEventData.data.id
                            };
                            ...
                            $.post(url, data, function (response) {
                                ...
                                if (response.warnings) {
                                 ...
                                 window.open(response.vide_url_operator);
                                // Refresh calendar event items.
                                $('#select-filter-item').trigger('change');
                            }, 'json').fail(GeneralFunctions.ajaxFailureHandler);
                        }
                ...
                GeneralFunctions.displayMessageBox(EALang.video_appointment_title,
                    EALang.warning_video_call, buttons);
    ...
    function _calendarEventClick(event, jsEvent, view) {
        ...
        if ($(this).hasClass('fc-unavailable') || $parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
            ...
            displayVideoCall = (($parent.hasClass('fc-custom') || $altParent.hasClass('fc-custom'))
                && GlobalVariables.user.privileges.appointments.videocall == true)
                ? '' : 'hide'; // Same value at the time.
            ...
            if (event.data.service.vide_active == "1") {
                html +=
                    '<button class="video-popover btn btn-primary ' + displayVideoCall + '">' + EALang.videocall + '</button>';
            }
            ...
            displayDelete = (GlobalVariables.user.privileges.appointments.delete == true)
                ? '' : 'hide';
            displayVideoCall = (GlobalVariables.user.privileges.appointments.videocall == true)
                ? '' : 'hide';
