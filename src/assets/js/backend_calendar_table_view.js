    ...
    function _bindEventHandlers() {
        ...
        $calendar.on('click', '.event', function (event) {
            ...
            if ($(this).hasClass('unavailability')) {
                const notes = '<strong>Notes</strong> ' + entry.notes;
                ...
                if (entry.service.vide_active == "1") {
                    html +=
                        '<button class="video-popover btn btn-primary">' + EALang.videocall + '</button>';
                }
                html +=
            ...
            const title = entry.is_unavailable !== '0' ? EALang.unavailable : entry.service.name + ' - '
        ...
        /**
         * Event: Popover Video Call Button "Click"
         *
         * Displays a prompt on whether the user wants the appointment to be deleted. If he confirms the
         * deletion then an ajax call is made to the server and deletes the appointment from the database.
         */
