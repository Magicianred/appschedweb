...
(function () {
    ...
    SystemSettings.prototype.save = function (settings) {
        ...
        $.post(postUrl, postData, function (response) {
            ...
            const workingPlan = BackendSettings.wp.get();
            $('.breaks tbody').empty();
            BackendSettings.wp.setup(workingPlan);
            BackendSettings.wp.timepickers(false);

            // Appschedweb refresh closing days and carousel settings.
            const closingDays = BackendSettings.cd.get();
            $('.closing-days tbody').empty();
            BackendSettings.cd.setup(closingDays);

            const carousel = BackendSettings.vc.get();
            $('.carousel-items tbody').empty();
            BackendSettings.vc.setup(carousel);
    ...
    SystemSettings.prototype.get = function () {
        ...
        let logo = $('#logo_base64').html();
        if (logo) {
            settings.push({
                name: 'company_logo',
                value: $('#logo_base64').html()
            });
        }
        ...
        if ($(".datepicker").length) {
            ...
            $( ".datepicker" ).each(function(i) {
                ...
                closingString[i] = '\''+date.getDate()+'-'+(date.getMonth()+1)+"-"+date.getFullYear()+'\'';
            });
            settings.push({
                name: 'company_closing_days',
                value: closingString.toString()
            });
        }

        settings.push({
            name: 'company_vide_carousel',
            value: JSON.stringify(BackendSettings.vc.get())
        });
    ...
    $(document).on('change', '#company-logo', function(e){
            let image = e.target.files[0];
            let encoded = toBase64(image);
            encoded.then((imageString) => {
                if (hasExtension('company-logo', ['.jpg', '.gif', '.png', '.svg'])) {
                    $('#logo-filename').html(image.name);
                    //console.log(imageString)
                    document.getElementById('logo_base64').innerHTML = imageString.substring(5);
                }
                else{
